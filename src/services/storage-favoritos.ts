...
/**
 * Grava a lista de favoritos na Async Storage
 */
export async function salvarLista(lista: Filme[]): Promise<void> {
  try {
    // Sava a lista de filmes no storage em formato de string
 ...
 * Retornar true se salvou com sucesso, false se o filme já estava nos favoritos
 */
export async function salvarFilmeFavorito(filme: Filme): Promise<boolean> {
  ...
  // portanto, adicionamos á lista de favoritos
...
export async function removerFilmeFavorito(id: number): Promise<void> {
  // Carregando a lista de favoritos já existente no storage
  const favoritos = await carregar();
