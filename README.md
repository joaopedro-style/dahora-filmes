...
- Uso de funções intermediárias em favoritos.tsx para acionar as funções de storage para salvar, apagar e apagarTudo.
...
- Criação de rota dinâmica: `app/detalhe/[id].tsx`
...
- Ajuste do Flatlist para renderização de componentes `<ItemVazio>` caso não tenha resultados na busca de filme
...
- Instalação do Axios (lib muito usada para cominicação com APIs)
...
## 03_navegação-expo-router-e-telas-privacidade-sobre
...
- Configuração das rotas usando componentes `Link` do expo-router
...
- Uso do ícones através do `@expo-vector-icons`
...
_Utilizando o `expo install` para garantir a instalação de dependências (ou libs) que sejam compativeis com a versão de SDK do Expo usada em seu projeto._

- Rfatoramos a estrutura e estilização do `App.tsx` usando os componentes: `SafeAreaProvider`, `SafeAreaView`, `StatusBar` (do expo), `View`, `Text` e `Button`
